...
namespace ITstudy.RedProjects
{
    ...
    public sealed partial class TowerOfHanoi : Page
    {
        ...
        private void NewGame(int towerHeight = -1)
        {
            ...
            // Allow the computer to solve the tower
        ...
        /// <summary>
        /// Automatically solve the current tower, resulting in a list of required moves which the user can step through
        /// </summary>
        private void GenerateSolution()
        {
            ...
            // Ensure the Gameboard is in a 'clean' state with all blocks on the starting tower and no blocks are in BlockState Available or Selected
            NewGame(TowerHeight);

            // Disable all user input on the gameboard, currently only the top most block on the first tower
            SetBlockIsClickable(0, 0, false);

            // Create the array that will contain all required moves, and set the index counter to 0
            SolutionMoves = new int[(int)(2 * Math.Pow(2, TowerHeight) - 1), 4];
            SolutionCurrentMove = 0;

            // Find the tower containing the largest block, and its height
            int startTowerNumber = FindLargestBlockOnTower0Or1(TowerHeight);
            int startTowerHeight = GetHighestBlock(startTowerNumber);

            // Return if the starting tower is not either 0 or 1, meaning the game was probably already completed
            if (startTowerNumber != 0 && startTowerNumber != 1)
            {
                Debug.WriteLine($"TowerOfHanoi: GenerateSolution() terminated because startTowerNumber was {startTowerNumber}");
                return;
            }

            // Find the correct index number for otherTower
            int othertTowerNumber = (startTowerNumber == 0) ? 1 : 0;

            // Create three Stack<int>'s, one for each of the towers, filled with the block widths on the Gameboard
            Stack<int> startTower = new Stack<int>();
            Stack<int> otherTower = new Stack<int>();
            Stack<int> endTower = new Stack<int>();
            for (int level = Gameboard.GetLength(1) - 1; level > 0; level--)
            {
                if (Gameboard[startTowerNumber, level].State == BlockState.Block) { startTower.Push(Gameboard[0, level].Width); }
                if (Gameboard[othertTowerNumber, level].State == BlockState.Block) { otherTower.Push(Gameboard[0, level].Width); }
                if (Gameboard[2, level].State == BlockState.Block) { endTower.Push(Gameboard[0, level].Width); }
            }
        ...
        /// <summary>
        /// Find the tower of a block by that blocks width, excluding tower 2. Starts by looking at the bottom blocks of the towers.
        /// If the block was found on tower 0 or 1, returns that value. If it was found on 2, the method will recursively call itself with a decremented blockWidth.
        /// Until the blockWidth reaches 1, return 2 meaning the game has actually already been completed, or -1 if something unforeseen went wrong.
        /// </summary>
        /// <param name="blockWidth">The width of the block to find</param>
        /// <returns>The tower where the block is located</returns>
        private int FindLargestBlockOnTower0Or1(int blockWidth)
        {
            int tower = -1;
            int bottom = Gameboard.GetLength(1) - 1;

            for (int i = 0; i < Gameboard.GetLength(0); i++)
            {
                if (Gameboard[i, bottom].Width == blockWidth)
                {
                    tower = i;
                }
            }

            Debug.WriteLine($"TowerOfHanoi: FindTowerOfBlockByBlockWidth() blockWidth = {blockWidth}, tower = {tower}");

            // If the current blockWidth is the smallest possible, return tower whatever its value
            if (blockWidth == 1)
            {
                return tower;
            }

            // If the block was not found in tower 0 or 1, start recursion with decremented value
            if (tower != 0 && tower != 1)
            {
                tower = FindLargestBlockOnTower0Or1(blockWidth - 1);
            }
        ...
        private void SolveRecursively(int towerHeight, Stack<int> fromTower, int fromTowerNumber, Stack<int> toTower, int toTowerNumber, Stack<int> otherTower, int otherTowerNumber)
        {
            ...
            if (towerHeight > 0)
            {
                ...
                LogSolutionMove(fromTowerNumber, TowerHeight - fromTower.Count, toTowerNumber, TowerHeight - toTower.Count, towerHeight);
